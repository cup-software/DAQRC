...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def load_config(self):
        result = QFileDialog.getOpenFileName(self, 'Load Configuration File', '',
    ...
    def boot_run(self):
        ...
        for daq in optlist:
            ...
            if mode > 0:
                cmd = onldaq_dir + \
                    '/bin/Linux5.14-GCC_11_3/%s %s -o "%s"' % (onlconsts.kEXESCRIPT,
    ...
    def exit_run(self):
        if self.RunState is onlconsts.kRUNNING:
    ...
    def update_runstate(self):
        ...
        if self.RunState == onlconsts.kRUNNING:
        ...
        if self.RunState == onlconsts.kRUNENDED:
            record = self.RunCatalogTable[self.RunNumber]
            etime = record['etime']
            if not etime:
                onlutils.send_command(self.RunSocket, onlconsts.kQUERYRUNINFO)
        ...
        summary += '     <b>DAQ State</b>: %s<br>' % onlconsts.kDAQSTATE[self.RunState]
    ...
    def set_runstate(self, state):
        ...
        if state == onlconsts.kDOWN:
        ...
        elif state == onlconsts.kRUNENDED:
            self.BootButton.setEnabled(False)
            self.ConfigButton.setEnabled(False)
            self.StartButton.setEnabled(False)
            self.StartButton.setStyleSheet("background-color: none")
            self.EndButton.setEnabled(False)
            self.EndButton.setStyleSheet("background-color: yellow")
        elif state == onlconsts.kPROCENDED:
            self.BootButton.setEnabled(False)
            self.ConfigButton.setEnabled(False)
            self.StartButton.setEnabled(False)
            self.StartButton.setStyleSheet("background-color: none")
